...
**Status**: ✅ **AUDIT COMPLETE** - Actionable checklist provided
...
**Key Findings:**
- ✅ **10 active migrations** identified (001-009 + 20250130 patch)
- ✅ **Brand ID UUID migration** appears complete (migrations 003, 005, 006)
- ⚠️ **Migration 006** (drop legacy brand_id TEXT columns) may not be applied yet - **VERIFY**
- ✅ **RLS policies** appear comprehensive in bootstrap migration
- ⚠️ **Code still uses `brand_id` (TEXT)** in some places - may need verification if migration 006 was applied
- ✅ **Core tables** (brands, content_items, auto_plans, scheduled_content, media_assets) are properly defined
...
- ✅ Branding migration completed (localStorage keys, Docker tags, user agent)
- ✅ Environment variable template created
- ✅ Connector scaffolds clearly marked
- ✅ Legacy server entry point verified (already using v2)
- ✅ Orphaned onboarding screens deleted
- ✅ Documentation archive completed

**Database-related claims:**
- ✅ Brand ID UUID migration mentioned in health docs (migrations 003, 005, 006)
- ✅ RLS policies implemented in bootstrap migration
- ✅ Media assets schema includes status column (migration 007)
...
|---|--------------|---------|------------|-------|
| 001 | `001_bootstrap_schema.sql` | Complete baseline schema (50+ tables, RLS policies) | ✅ **Safe** | Single authoritative baseline. Safe to run on empty DB. Includes all core tables. |
| 002 | `002_create_brand_guide_versions.sql` | Brand guide version history table | ✅ **Safe** | Idempotent (IF NOT EXISTS). Already in bootstrap, but safe for existing DBs. |
| 003 | `003_fix_brand_id_persistence_schema.sql` | Add `brand_id_uuid` columns to 10 persistence tables | ⚠️ **Important** | Step 1 of brand_id TEXT → UUID migration. Adds columns, backfills data. |
| 004 | `004_activate_generation_logs_table.sql` | Activate generation_logs table | ✅ **Safe** | Idempotent (IF NOT EXISTS). Table used by server/routes/agents.ts. |
...
#### ✅ Safe Migrations (001, 002, 004, 008, 010)
- Use `IF NOT EXISTS` patterns
- Idempotent
- No data loss risk
- Can be applied multiple times safely

#### ⚠️ Important Migrations (003, 005, 007, 009)
...
**Migration Status:**
- ✅ Defined in `001_bootstrap_schema.sql`
- ✅ Migration 009 merges legacy fields into `brand_kit`
- ⚠️ **Action**: Verify migration 009 applied (legacy fields should still exist but be deprecated)

#### ✅ `brand_guide_versions` Table
**Code Expectations:**
- `id` (UUID)
...
**Migration Status:**
- ✅ Defined in `001_bootstrap_schema.sql`
- ✅ Migration 002 creates it (idempotent)
- ✅ Migration 20250130 patches it (idempotent)
- ✅ **Status**: Should exist
...
**Migration Status:**
- ✅ Defined in `001_bootstrap_schema.sql`
- ✅ Uses `brand_id` UUID (not TEXT)
- ✅ **Status**: Should exist with correct schema
...
- ✅ Defined in `001_bootstrap_schema.sql`
- ✅ Uses `brand_id` UUID
- ✅ Migration 008 documents this as canonical (no schema changes)
- ✅ **Status**: Should exist. Code uses `auto_plans` (not `monthly_content_plans`)
...
- ✅ Defined in `001_bootstrap_schema.sql`
- ✅ Uses `brand_id` UUID
- ✅ Migration 008 documents this as canonical
- ✅ **Status**: Should exist

#### ⚠️ `media_assets` Table
**Code Expectations:**
- `id` (UUID)
- `brand_id` (UUID) - Primary identifier
- `tenant_id` (UUID)
...
**Migration Status:**
- ✅ Defined in `001_bootstrap_schema.sql`
- ⚠️ **Migration 007 adds `status` column** - **VERIFY THIS EXISTS**
- ⚠️ **Migration 007 adds INSERT/UPDATE RLS policies** - **VERIFY THESE EXIST**
- ✅ Uses `brand_id` UUID (not TEXT)
...
**Action Required:**
- ✅ Verify `status` column exists in `media_assets`
- ✅ Verify INSERT/UPDATE RLS policies exist

#### ⚠️ Persistence Schema Tables (10 tables)
...
**Migration Status:**
- ✅ Migration 003: Adds `brand_id_uuid` columns, backfills data
- ⚠️ **Migration 005**: Updates RLS policies to use `brand_id_uuid`, adds FK constraints - **VERIFY APPLIED**
- 🔴 **Migration 006**: Drops `brand_id` TEXT columns - **VERIFY STATUS** (may not be applied yet)
...
**Action Required:**
- ✅ Verify migration 005 applied (RLS policies use `brand_id_uuid`, FKs exist)
- ⚠️ **Verify migration 006 status** - If applied, `brand_id` TEXT columns should be dropped. If not applied, they may still exist.
...
**Status**: ✅ **Correct** - Defined in `001_bootstrap_schema.sql`

#### ⚠️ `media_assets` RLS
**Migration 007 adds:**
- ✅ INSERT policy: Brand members can insert
- ✅ UPDATE policy: Brand members can update
- ✅ SELECT policy: Should exist in bootstrap (verify)

**Action**: Verify INSERT/UPDATE policies exist

#### ⚠️ Persistence Schema Tables RLS
**Migration 005 updates:**
- ✅ Policies should use `brand_id_uuid` (not `is_brand_member_text()`)
- ✅ Policies should check `brand_members` table directly
...
**Action**: 
- ✅ Verify persistence table policies use `brand_id_uuid`
- ✅ Verify `is_brand_member_text()` is dropped (if migration 006 applied)

#### ✅ `brand_guide_versions` RLS
**Status**: ✅ **Correct**
...
**Status**: ✅ **Correct** - Defined in migrations 001, 002, 20250130

### RLS Helper Functions

| Function | Purpose | Status | Notes |
|----------|---------|--------|-------|
| `is_brand_member_text(brand_id_param TEXT)` | Checks brand membership for TEXT brand_id | ⚠️ **Should be dropped** | Used by persistence tables before migration 005. Should be dropped if migration 006 applied. |
| `is_workspace_member(workspace_id_param TEXT)` | Checks workspace membership | ✅ **Active** | Used by `milestones` table. Still needed. |
| `update_updated_at()` | Trigger function for timestamps | ✅ **Active** | Used by all tables with `updated_at` columns. |
...
**⚠️ IMPORTANT**: Only apply migrations that haven't been applied yet. Check status first.
...
5. **Apply migration 005 (finalize brand_id_uuid) if needed:**
   ...
   ```
   **⚠️ CRITICAL**: This updates RLS policies. Verify policies after applying.
...
9. **🔴 DO NOT APPLY migration 006 yet:**
   - **Only apply after:**
...
**⚠️ MORE CONSERVATIVE - Non-destructive verification first**

#### Step 1: Backups

1. **Before applying ANY new migration:**
   - ✅ Create a database backup/snapshot in Supabase Dashboard
...
**⚠️ Apply only migrations that haven't been applied yet**
...
5. **Migration 005**: ⚠️ **Apply during maintenance window** - Updates RLS policies
6. **Migration 007**: ⚠️ **Apply during maintenance window** - Adds column and RLS policies
7. **Migration 009**: ⚠️ **Apply during maintenance window** - Data migration
8. **Migration 20250130**: Idempotent, safe to apply
9. **Migration 006**: 🔴 **DO NOT APPLY YET** - Only after full code migration verified
...
|-----------|---------|------|--------|
| 001 | Bootstrap schema | ✅ Safe | Should be applied |
| 002 | Brand guide versions | ✅ Safe | Should be applied |
| 003 | Add brand_id_uuid columns | ⚠️ Important | Should be applied |
| 004 | Activate generation_logs | ✅ Safe | Should be applied |
| 005 | Finalize brand_id_uuid (FKs, RLS) | ⚠️ Important | **VERIFY APPLIED** |
| 006 | Drop legacy brand_id TEXT | 🔴 Dangerous | **VERIFY STATUS** |
| 007 | Media assets status + RLS | ⚠️ Important | **VERIFY APPLIED** |
| 008 | Documentation only | ✅ Safe | No action needed |
| 009 | Consolidate brand_guide fields | ⚠️ Important | Should be applied |
| 20250130 | Brand guide versions patch | ✅ Safe | Should be applied |
...
|----------------|------------|------------------|-------|
| Core tables | ✅ Correct | Direct brand_members check | All have proper RLS |
| `media_assets` | ⚠️ Verify | Direct brand_members check | Migration 007 adds INSERT/UPDATE policies |
| Persistence tables | ⚠️ Verify | Should use `brand_id_uuid` | Migration 005 updates policies |
| `brand_guide_versions` | ✅ Correct | Direct brand_members check | Immutable (no UPDATE/DELETE) |

---

## Critical Action Items Summary

1. **✅ Verify migration 005 applied** - RLS policies should use `brand_id_uuid`
2. **⚠️ Verify migration 006 status** - If applied, `brand_id` TEXT columns should be dropped
3. **✅ Verify migration 007 applied** - `media_assets.status` column should exist
4. **✅ Test RLS policies** - Ensure brand isolation works correctly
5. **⚠️ Verify persistence schema** - All 10 tables should have `brand_id_uuid` columns populated
