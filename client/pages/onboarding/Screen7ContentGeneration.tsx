...
export default function Screen7ContentGeneration() {
  ...
  const startGeneration = async () => {
    try {
      ...
      // ✅ Check if auto-workflow already generated content (from Screen3AiScrape)
      // NOTE: Key is brand-specific to support multi-brand / agency onboarding
      const workflowAlreadyCompleted = localStorage.getItem(`postd:onboarding:${brandId}:workflow_completed`) === "true";
      if (workflowAlreadyCompleted) {
        // Content already generated by auto-workflow, show quick success animation
        console.log("[OnboardingWorkflow] Screen7 skipped - workflow_completed=true for brandId=" + brandId);
        
        // Quick animation through steps
        ...
        fire({
          ...
          spread: 80,
          origin: { y: 0.5 },
          colors: ["#632bf0", "#c084fc", "#e2e8f0", "#a855f7", "#12b76a"],
        });
      ...
      // NOTE: Only called if auto-workflow did NOT already complete
      console.log("[OnboardingWorkflow] Screen7 calling content-plan API for brandId=" + brandId);
      ...
      try {
        ...
        if (data.success && data.contentPlan) {
          ...
          const contentPackage = {
            ...
            brandId, // Include brandId for validation
          };
          // ✅ FIX: Use brand-specific key (prevents cross-brand leakage)
          localStorage.setItem(`postd:onboarding:${brandId}:content_package`, JSON.stringify(contentPackage));
          // Also mark workflow as completed (in case it wasn't set by Screen3)
          localStorage.setItem(`postd:onboarding:${brandId}:workflow_completed`, "true");
          console.log("[OnboardingWorkflow] Content package stored", { brandId, itemCount: data.contentPlan.items?.length || 0 });
        }
      } catch (apiError) {
